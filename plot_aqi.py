# -*- coding: utf-8 -*-
"""
Created on Tue Mar 17 21:28:56 2020
...
#csv file is having data of PM2.5 for every hour.
def avg_data_2013():
    temp_i=0
    average=[]
    for rows in pd.read_csv('data/AQI/aqi2013.csv',chunksize=24):#takeing the value of 24 column that represnt data of 1 day
        add_var=0
        avg=0.0
        data=[]
        df=pd.DataFrame(data=rows)
        for index,row in df.iterrows():#appending PM2.5 value in data list
            data.append(row['PM2.5'])
        for i in data:#cleaning the data
            if type(i) is float or type(i) is int:
                add_var=add_var+i
            elif type(i) is str:#cleaning the data
        ...
        average.append(avg)#average list value of pm2.5 for year 2013
    return average
